...
/**
  * A resettable Flink SourceFunction to generate Sensors with random temperature values.
  *
  ...
  */
class ResettableSensorSource extends RichParallelSourceFunction[Sensor] with CheckpointedFunction {

  // flag indicating whether source is still running.
  var running: Boolean = true

  // the last emitted sensor readings
  var readings: Array[Sensor] = _

  // state to checkpoint last emitted readings
  var sensorsState: ListState[Sensor] = _

  /** run() continuously emits Sensors by emitting them through the SourceContext. */
    ...
    while (running) {
      ...
      srcCtx.getCheckpointLock.synchronized {
        ...
        for (i <- readings.indices) {
          ...
          val newReading = Sensor(reading.id, newTime, newTemp)
  ...
  override def initializeState(ctx: FunctionInitializationContext): Unit = {
    ...
    this.sensorsState = ctx.getOperatorStateStore.getUnionListState(
      new ListStateDescriptor[Sensor]("sensorsState", classOf[Sensor]))
