package cn.beginman.flinkapp.c5

import java.sql.{Connection, DriverManager, PreparedStatement}
import java.util.Properties

import cn.beginman.flinkapp.c5.util.{DerbyReader, DerbySetup}
...
object IdempotentSinkFunctionExample {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val celsiusReadings: DataStream[Sensor] = sensorData
      // convert Fahrenheit to Celsius using an inlined map function
      .map( r => Sensor(r.id, r.timestamp, (r.temperature - 32) * (5.0 / 9.0)) )
...
/**
  * Sink that upserts Sensors into a Derby table that is keyed on the sensor id.
  *
  ...
  */
class DerbyUpsertSink extends RichSinkFunction[Sensor] {
  ...
  override def invoke(r: Sensor, context: Context[_]): Unit = {
