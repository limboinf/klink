...
import cn.beginman.flinkapp.c5.util.{DerbySetup, DerbyWriter}
...
/**
  * AsyncFunction以非阻塞方式通过JDBC查询。
  *
  * 由于JDBC接口不支持异步查询，因此启动单个线程以并发查询Derby并以非阻塞方式处理查询结果。
  */
class DerbyAsyncFunction extends AsyncFunction[Sensor, (String, String)] {
...
/**
  ...
  */
class DerbySyncFunction extends RichMapFunction[Sensor, (String, String)] {
  ...
  override def map(reading: Sensor): (String, String) = {
