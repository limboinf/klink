...
object LateDataHandling {
  ...
  val lateReadingsOutput: OutputTag[Sensor] =
    new OutputTag[Sensor]("late-readings")
  ...
  def main(args: Array[String]): Unit = {
    ...
    val outOfOrderReadings: DataStream[Sensor] = env
...
//    updateForLateEventsWindow(outOfOrderReadings)
  ...
  def filterLateReadings(readings: DataStream[Sensor]): Unit = {
    // re-direct late readings to the side output
    val filteredReadings: DataStream[Sensor] = readings
      .process(new LateReadingsFilter)

    // retrieve late readings
    val lateReadings: DataStream[Sensor] = filteredReadings
  ...
  def sideOutputLateEventsWindow(readings: DataStream[Sensor]): Unit = {
    ...
    val countPer10Secs: DataStream[(String, Long, Int)] = readings
      ...
      .process(new ProcessWindowFunction[Sensor, (String, Long, Int), String, TimeWindow] {

        override def process(
            id: String,
            ctx: Context,
            elements: Iterable[Sensor],
  ...
  /** Count reading per tumbling window and update results if late readings are received.
    * Print results. */
  def updateForLateEventsWindow(readings: DataStream[Sensor]): Unit = {
...
class LateReadingsFilter extends ProcessFunction[Sensor, Sensor] {
...
class UpdatingWindowCountFunction
    extends ProcessWindowFunction[Sensor, (String, Long, Int, String), String, TimeWindow] {
...
/** A MapFunction to shuffle (up to a max offset) the timestamps of Sensors to produce out-of-order events. */
class TimestampShuffler(maxRandomOffset: Int) extends MapFunction[Sensor, Sensor] {

  lazy val rand: Random = new Random()

  override def map(r: Sensor): Sensor = {
    val shuffleTs = r.timestamp + rand.nextInt(maxRandomOffset)
    Sensor(r.id, shuffleTs, r.temperature)
  }
}
